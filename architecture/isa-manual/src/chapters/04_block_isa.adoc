...
+
+NOTE: In v0.3, `BSTART.*` / `BSTOP` are treated as **block boundary markers** for block init/commit. They may appear as markers/descriptors in the program stream, but there must be no architectural state-modifying *payload* instructions outside block execution.
...
* Elementwise/base (tile-tile): `TADD=0x000`, `TSUB=0x001`, `TMUL=0x002`, `TDIV=0x003`, `TMAX=0x004`, `TMIN=0x005`,
  `TAND=0x006`, `TOR=0x007`, `TXOR=0x008`, `TSHL=0x009`, `TSHR=0x00A`, `TRELU=0x00D`, `TPRELU=0x00E`, `TCVT=0x00F`.
* Elementwise TOPS (tile-scalar, scalar RHS via `B.IOR`, broadcast):
  `TADDS=0x040`, `TSUBS=0x041`, `TMULS=0x042`, `TDIVS=0x043`, `TMAXS=0x044`, `TMINS=0x045`, `TANDS=0x046`,
  `TORS=0x047`, `TXORS=0x048`, `TSHLS=0x049`, `TSHRS=0x04A`.
  - NOTE (toolchain convention): place the scalar RHS binding in the first `B.IOR` entry where practical, even though
    implementations must accept any `B.IOR` position per op-defined parsing.
* Row/column reductions: `TROWMAX=0x020`, `TROWMIN=0x021`, `TROWSUM=0x022`, `TCOLMAX=0x024`, `TCOLMIN=0x025`,
  `TCOLSUM=0x026`.
* Reduce expand: `TCOLEXPAND=0x0C0`, `TROWEXPAND=0x0C1`.
* Scalar splat/broadcast: `TEXPANDS=0x045`.
...
Non-canonical legacy note:

* `TCOLEXPAND=0x027` is non-canonical in strict v0.3 and MUST NOT appear in canonical outputs.
* Unassigned `TileOp10` values are **non-canonical** and MUST NOT appear in canonical outputs; canonical toolchains MUST reject them. Hardware behavior for such values is unspecified.
...
* The body MUST terminate at a block terminator marker. Valid terminators are `BSTOP` / `C.BSTOP` and `BSTART.*` / `C.BSTART.*`.
  ...
  - Any `BSTART` encountered inside the body stream acts as a terminator trigger (deterministic), not as an architecturally executed successor from within the body.
